...
search_path = "html/docs.gl.search.js"
...
footer_fp = open(footer_path)
footer = footer_fp.read()
footer_fp.close()

search_fp = open(search_path)
...
for command in index_commands_version:
    ...
    for version in all_major_versions:
        ...
        if version in all_major_versions_available:
            # change
            index_versions_commands += "<span class='versioncolumn'><a href='/" + version + "/" + alias + ".html'>" + version + "</a></span>"
...
for command in glsl_index_commands_version:
    ...
    for version in all_major_versions:
        ...
        if version in all_major_versions_available:
            if version[0:2] == "sl":
                # change
                glsl_index_versions_commands += "<span class='slversioncolumn'><a href='" + version + "/" + alias + ".html'>glsl" + version[2:3] + "</a></span>"
                # glsl_index_versions_commands += "<span class='slversioncolumn'><a href='" + version + "/" + alias + "'>glsl" + version[2:3] + "</a></span>"
            else:
                # change
                glsl_index_versions_commands += "<span class='slversioncolumn'><a href='" + version + "/" + alias + ".html'>glsl-es" + version[2:3] + "</a></span>"
...
# Hope this works!
    ...
    for command in commands_version_flat:
        ...
        # change
        output_path = output_dir + version_dir + "/" + command + ".html"
    ...
    if os.path.exists("html/404.html"):
        ...
        output = open(output_dir + version + "/404.html", "w")
        output_string = output_html
        if args.buildmode == 'full':
            output_string = htmlmin.minify(output_html, remove_comments=True, reduce_boolean_attributes=True, remove_optional_attribute_quotes=False).encode('ascii', 'xmlcharrefreplace').decode('ascii')
        output.write(output_string)
